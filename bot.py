...
BOT_TOKEN = os.environ.get('BOT_TOKEN', '8448331382:AAEpqo2x_-c-CLv1baAHKJ88e6hr7Ejk2sk')
...
def create_payment(update, context, user_id, amount, payment_type, details):
    ...
    if payment_id:
        ...
        update.message.reply_text(payment_text)
        return True
    return False
    ...
    try:
        ...
        if len(payment_data) >= 3:
            ...
            if payment and not payment[6]:
                ...
                update.message.reply_text("Платеж уже обработан или не найден.")
        else:
            update.message.reply_text("Неверный формат платежа.")
    except Exception as e:
        update.message.reply_text("Ошибка обработки платежа.")
...
def register_nickname(update, context):
...
def register_wallet(update, context):
...
# Выбор комнаты
def handle_room_selection(update, context):
    text = update.message.text
    ...
    if text == '🔙 Назад':
        ...
        update.message.reply_text("Пожалуйста, выберите комнату из списка:", reply_markup=rooms_keyboard())

# Вход в комнату
def join_room(update, context, room_name):
    user_id = update.effective_user.id
    rooms = db.get_rooms()
...
# Создание комнаты
def create_room_name(update, context):
    ...
    for room in rooms:
        if room[1].lower() == room_name.lower():
            update.message.reply_text("Комната с таким названием уже существует. Выберите другое название:")
            return CREATE_ROOM_NAME
...
def create_room_type(update, context):
    text = update.message.text
...
def create_room_persistence(update, context):
    text = update.message.text
    
    if text == '🔙 Назад':
        update.message.reply_text("Выберите тип комнаты:", reply_markup=room_type_keyboard())
        return CREATE_ROOM_TYPE
    ...
    if context.user_data['is_private']:
        update.message.reply_text("Введите пароль для приватной комнаты:")
        return CREATE_ROOM_PASSWORD
    else:
        return finish_room_creation(update, context)

def create_room_password(update, context):
    text = update.message.text
    
    if text == '🔙 Назад':
        update.message.reply_text("Сообщения должны сохраняться или удаляться после прочтения?", reply_markup=room_persistence_keyboard())
        return CREATE_ROOM_PERSISTENCE
    
    context.user_data['password'] = text
    return finish_room_creation(update, context)

def finish_room_creation(update, context):
    user_id = update.effective_user.id
    ...
    user = db.get_user(user_id)
    if user[6] < 2.0:
        payment_success = create_payment(
...
# Передача USD
def transfer_user(update, context):
    nickname = update.message.text
...
# Пополнение баланса
def handle_payment_amount(update, context):
    try:
        ...
        update.message.reply_text("Пожалуйста, выберите сумму из предложенных:")
        return PAYMENT_CONFIRMATION

# Админ панель
def admin_action(update, context):
    text = update.message.text
...
def mute_duration(update, context):
    try:
...
def main():
    ...
    # Обработчик команды /start с регистрации
