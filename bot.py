...
async def finish_room_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = db.get_user(user_id)
    if user[6] < 2.0:
        await update.message.reply_text("Недостаточно средств для создания комнаты. Нужно 2 USD.")
        return ConversationHandler.END
    ...
    if db.create_room(room_name, user_id, is_ephemeral, is_private, password):
        await update.message.reply_text(f"Комната '{room_name}' успешно создана!", reply_markup=main_keyboard())
    else:
